...
**로그를 정규화하고 → 위협을 탐지하고 → 인시던트를 생성하고 → 대응 및 기록까지 이어지는 보안 운영 자동화 흐름**을 직접 구현하는 것입니다.
...
특히 브루트포스, 계정 탈취, 권한 상승, 내부 이동 같은 대표적인 공격 시나리오가 실제로는 어떤 식으로 탐지 로직으로 연결되는지 궁금했고, 이를 이해해보자 이 프로젝트를 진행하게 되었습니다.
...
또한 완전한 실시간 스트리밍 처리보다는 로컬 파일 기반 분석에 가깝고, 실제 SIEM, EDR, IAM, 방화벽과의 완전한 연동까지는 구현하지 못했습니다.

---

## 개선 방향

현재는 로컬 파일 기반으로 로그를 처리하는 방식이기 때문에, 실시간 로그 수집 파이프라인과 연결해서 조금 더 실제 운영 환경에 가까운 형태로 확장할 필요가 있어보입니다.

지금은 일부 탐지 로직이 키워드 기반이나 임계값 기반으로 동작하기 때문에, 정상 행위와 공격 행위를 더 잘 구분할 수 있도록 예외 처리와 탐지 조건을 더 정교하게 다듬는 것이 좋아보입니다.

대응 부분도 상태 파일 기록이나 기본적인 연동 구조 중심이기 때문에, 추후에는 SIEM, EDR, 방화벽, IAM 같은 외부 보안 솔루션과 실제로 연결되는 방향까지 구현해 볼 필요성을 느꼈습니다.

마지막으로 현재는 결과를 파일 중심으로 확인하는 구조이지만, 탐지 결과와 대응 내역을 한눈에 볼 수 있도록 대시보드나 리포트 자동 생성 기능도 추가하는 것이 좋다고 생각했습니다.
...
예를 들어 로그인 실패 하나만 본다면 단순 실패일 수 있지만, 짧은 시간 안에 반복되고 그 뒤에 성공까지 이어지면 계정 탈취 의심 시나리오가 될 수 있다는 걸 구현을 하면서 더 확실하게 이해할 수 있었습니다.

또한 로그 형식이 제각각이면 탐지 로직을 만들기 어렵기 때문에, 먼저 이벤트를 공통 구조로 정규화하는 과정이 왜 필요한지 느낄 수 있었습니다.

그리고 탐지에서 끝나는 것이 아닌, 탐지 결과를 인시던트로 남기고 차단·격리·알림 같은 대응 흐름까지 연결해야 조금 더 보안 자동화 시스템다운 형태가 된다는 점도 새롭게 배울 수 있었습니다.

이 프로젝트를 통해 규칙 기반 탐지 시스템의 구조, 상태 기반 탐지의 필요성, 보안 운영 관점에서 기록과 대응이 왜 중요한지 이해할 수 있었습니다.
...
이 프로젝트는 로그를 자동 분석하여 보안 위협을 탐지하고, 대응과 기록까지 연결하는 규칙 기반 보안 자동화 시스템입니다.  
단순한 로그 분석기를 넘어, 정규화 → 탐지 → 인시던트 생성 → 대응 → 상태 저장 흐름을 구현한 프로젝트라는 점에 의미가 있습니다.
